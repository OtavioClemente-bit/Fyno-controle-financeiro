...
class _ReceiptItemsRepo {
  ...
  static Future<List<Map<String, Object?>>> searchProducts(String query) async {
    ...
    if (q.isEmpty) {
      ...
      SELECT
        norm_name,
        MAX(COALESCE(display_name, raw_name)) AS display_name,
        COUNT(*) AS count,
...
class _ProductPricesPageState extends State<ProductPricesPage> {
  ...
  Widget build(BuildContext context) {
    ...
    final compact = MediaQuery.sizeOf(context).width < 360 || textScale > 1.25;
    ...
    final historySliver = selNorm == null
        ? SliverToBoxAdapter(
            child: _EmptyHint(color: cs.onSurface.withValues(alpha: 0.7)),
          )
        : _history.isEmpty
        ? SliverToBoxAdapter(
            child: _loading
                ? const SizedBox.shrink()
                : _EmptyHistoryHint(color: cs.onSurface.withValues(alpha: 0.7)),
          )
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(compact ? 'Preços' : 'Preços de produtos'),
        actions: compact
            ? [
                IconButton(
                  tooltip: 'Limpar',
      ...
      floatingActionButton: compact
          ...
          : FloatingActionButton.extended(
              ...
              icon: const Icon(Icons.add),
              label: Text(
                selNorm == null ? 'Adicionar produto' : 'Adicionar preço',
              ),
            ),
      body: SafeArea(
          ...
          children: [
            TextField(
              ...
              decoration: InputDecoration(
                ...
                hintText: compact
                    ? 'Pesquisar'
                    : 'Pesquisar produto (ex: banana, arroz, leite)',
