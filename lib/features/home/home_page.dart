...
class HomePage extends StatefulWidget {
  const HomePage({super.key, this.showQuickAddFab = true});
...
class _HomePageState extends State<HomePage> {
  ...
  List<TxItem> _applyFiltersForChart(List<TxItem> input) {
    ...
    return input.where((tx) {
      if (tx.isCreditCardBillPayment) return false;
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 8),
                    // IMPORTANTE: garantir que NÃO existam categorias duplicadas.
                    // Se houver 2 itens com o mesmo value (ex: "Salário"), o Flutter
                    // dispara um assert e a tela trava.
                    Builder(
                      builder: (_) {
                        final catsUnique = <String>[];
                        final seen = <String>{};
                        for (final c in categories) {
                          final v = c.trim();
                          if (v.isEmpty) continue;
                          if (seen.add(v)) catsUnique.add(v);
                        }

                        // Se a categoria atual não existir mais na lista (ex: foi removida),
                        // não setamos initialValue para evitar o mesmo assert.
                        final safeCat =
                            (cat != null && catsUnique.contains(cat))
                            ? cat
                            : null;
  ...
  Widget build(BuildContext context) {
    ...
    final stickyHeaderExtent = textScale > 1.25 ? 224.0 : 194.0;
    final compactAppBar =
          ...
          slivers: [
            ...
            SliverPersistentHeader(
              ...
              delegate: _StickyHeaderDelegate(
                minExtent: stickyHeaderExtent,
                maxExtent: stickyHeaderExtent,
    ...
    return Material(
      ...
      child: InkWell(
        ...
        child: Padding(
          ...
          child: LayoutBuilder(
            builder: (context, constraints) {
              final showArrow = constraints.maxWidth >= 110;
              return Row(
                children: [
                  Icon(icon, size: 18),
                  const SizedBox(width: 8),
...
class _PremiumTxCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final isIncome = tx.isIncome;
    final isCardPayment = tx.isCreditCardBillPayment;
    final accent = isCardPayment
        ? cs.onSurfaceVariant
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      '${FormatBR.date(tx.date)} • ${isCardPayment ? 'Quitação do cartão' : meta.name}${(tx.receiptImagePath != null) ? '  •  📷' : ''}',
                      maxLines: 1,
                      overflow: TextOverflow.ellipsis,
                      style: TextStyle(
                        fontSize: 12.2,
          ...
          children: [
            ...
            LayoutBuilder(
              builder: (context, constraints) {
                  ...
                  children: [
                    for (final e in entries.take(6))
                      _AllocRow(
                        ...
                        selected:
                            hasSelection && _norm(selectedKey!) == _norm(e.key),
                ...
                // On compact phones the donut and legend need the full card
                // width. Stacking them prevents long category names from
                // colliding with their percentages, including at large text
                // scales.
