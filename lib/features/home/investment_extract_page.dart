...
class _InvestmentExtractPageState extends State<InvestmentExtractPage> {
  ...
  String _investmentKindOf(TxItem tx) {
    ...
    if (note.isNotEmpty) {
      ...
      if (m != null) {
        // Os metadados ficam na mesma linha, separados por “•”. Apenas o
        // primeiro trecho representa o tipo usado nos filtros e no gráfico.
                    ...
                    Container(
                      ...
                      child: Row(
                        ...
                        children: [
                          ...
                          Expanded(
                            child: Text(
                              'Esta tela mostra somente entradas de investimentos. Use o botão de lixeira para remover um lançamento.',
                              style: TextStyle(
                                height: 1.35,
                                color: cs.onSurfaceVariant,
                                fontSize: 12.5,
...
class _TxCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final note = (tx.note ?? '').trim();
    final noteLines = note
        .split('\n')
        .map((line) => line.trim())
        .where((line) => line.isNotEmpty)
        .toList(growable: false);
    final origin = noteLines.cast<String?>().firstWhere(
      (line) => line!.toLowerCase().startsWith('origem:'),
      orElse: () => null,
    );
    final investmentLine = noteLines.cast<String?>().firstWhere(
      (line) => line!.toLowerCase().startsWith('investimento:'),
      orElse: () => null,
    );
    final details = investmentLine
        ?.split('•')
        .skip(1)
        .map((part) => part.trim())
    ...
    final supportingText = <String>[
      ...
      ...noteLines.where((line) => line != origin && line != investmentLine),
    ].join('  •  ');
      ...
      child: Padding(
        ...
        child: Row(
          ...
          children: [
            CircleAvatar(
              radius: 22,
              backgroundColor: cs.primaryContainer.withValues(alpha: .65),
                ...
                children: [
                  LayoutBuilder(
                    builder: (context, constraints) {
                      ...
                      final stacked =
                          constraints.maxWidth < 230 || textScale > 1.2;
                      final title = Text(
                        tx.title,
                        maxLines: 2,
                        overflow: TextOverflow.ellipsis,
                      ...
                      if (stacked) {
                        return Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [title, const SizedBox(height: 4), amount],
                        );
                      }
                      return Row(
                        crossAxisAlignment: CrossAxisAlignment.start,
...
class _Tag extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final cs = Theme.of(context).colorScheme;
    final maxWidth = math.min(230.0, MediaQuery.sizeOf(context).width * .58);
